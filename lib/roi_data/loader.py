...
class RoiDataLoader(data.Dataset):
    def __init__(self, roidb, num_classes, training=True):
        ...
        self.aug = SSDAugmentation()

    def __getitem__(self, index_tuple):
        index, ratio = index_tuple
        single_db = [self._roidb[index]]
        blobs, valid = get_minibatch(single_db, self._num_classes, self.aug)
        ...
        blobs['data_aug'] = blobs['data_aug'].squeeze(axis=0)
        blobs['data_aug1'] = blobs['data_aug1'].squeeze(axis=0)
        blobs['data_rot'] = blobs['data_rot'].squeeze(axis=0)
...
def collate_minibatch(list_of_blobs):
    ...
    for blobs in list_of_blobs:
        lists.append({'data' : blobs.pop('data'),
                      'data_aug' : blobs.pop('data_aug'),
                      'data_aug1': blobs.pop('data_aug1'),
                      'rois' : blobs.pop('rois'),
                      'labels' : blobs.pop('labels'),
                      'data_rot': blobs.pop('data_rot'),
                      'rot_inds': blobs.pop('rot_inds')})
