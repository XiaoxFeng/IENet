...
class JsonDataset(object):
    ...
    def _add_proposals_from_file(
        ...
        for i, entry in enumerate(roidb):
            ...
            ##############################sh change order 0123 -->1032
            # s_bbox = proposals['boxes'][i]
            # boxes = np.concatenate((s_bbox[:, 1][:, np.newaxis], s_bbox[:, 0][:, np.newaxis],
            #                         s_bbox[:, 3][:, np.newaxis], s_bbox[:, 2][:, np.newaxis]), axis=1)
            ##############################end
            ############orginal
            boxes = proposals['boxes'][i]#[:, (1,0,3,2)]
            ##############
            # Sanity check that these boxes are for the correct image id
            ###########original
            #assert int(entry['id']) == int(proposals[id_field][i][0][-12:])
            assert int(entry['id']) == int(proposals[id_field][i])
            ###########sh
