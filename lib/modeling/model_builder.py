...
from model.pcl.pcl import OICR, OICRLosses
...
class Generalized_RCNN(nn.Module):
    ...
    def _forward(self, data, data_aug, data_aug1, rois, labels, data_rot, rot_inds):
        ...
        #blob_conv_o, blob_conv_aug = blob_conv.chunk(2, dim=0)
        ...
        refine_score, refine_score_eq = self.Box_Refine_Outs(feat, box_feat_FT)
        if cfg.MODEL.WITH_FRCNN:
        ...
        if self.training:
            ...
            for i_refine, refine in enumerate(refine_score[:3]):
                if i_refine == 0:
                    pcl_output = OICR(boxes, mil_score, im_labels, refine, rot_inds)
                else:
                ...
                Eq_refine_loss = self.Refine_Losses[i_refine](
                    refine_score_eq[i_refine],
                    Variable(torch.from_numpy(pcl_output['labels'])).to(device),
                    Variable(torch.from_numpy(pcl_output['cls_loss_weights'])).to(device),
                    Variable(torch.from_numpy(pcl_output['gt_assignment'])).to(device))
                ...
                return_dict['losses']['refine_loss_eq%d' % i_refine] = Eq_refine_loss.clone()

            eq_loss = self.EQ_loss(
                    refine_score[3],
                    Variable(torch.from_numpy(pcl_output['rot_labels'])).to(device),
                    Variable(torch.from_numpy(pcl_output['cls_loss_weights'])).to(device),
                    Variable(torch.from_numpy(pcl_output['gt_assignment'])).to(device))

            #Eq_refine_loss = self.EQ_loss(refine_score[0], refine_score_2[0])

            return_dict['losses']['Eq_loss%d' % 3] = eq_loss.clone()

            if cfg.MODEL.WITH_FRCNN:
                labels, cls_loss_weights, bbox_targets, bbox_inside_weights, \
                    bbox_outside_weights, rot_labels = fast_rcnn_heads.get_fast_rcnn_targets(
