...
from model.OICR.OICR import get_proposal_clusters,  _get_highest_score_proposals_rot
import nn as mynn
...
def get_fast_rcnn_targets(boxes, refine_score, im_labels, rot_inds):
    cls_prob = refine_score[-2].data.cpu().numpy()
    rot_prob = refine_score[-1].data.cpu().numpy()

    if cls_prob.shape[1] != im_labels.shape[1]:
        cls_prob = cls_prob[:, 1:]
        #cls_prob += rot_prob[rot_inds]
    eps = 1e-9
    ...
    proposals = _get_highest_score_proposals_rot(boxes.copy(), cls_prob.copy(), im_labels.copy(),
                                                 rot_prob[:, rot_inds[0]], rot_inds)
    #proposals = _fast_get_highest_score_proposals(boxes.copy(), cls_prob.copy(), rot_prob.copy(), im_labels.copy(), rot_inds)
    ...
    return labels.reshape(-1).astype(np.int64).copy(), \
           ...
           rot_labels.reshape(-1).astype(np.int64).copy()
