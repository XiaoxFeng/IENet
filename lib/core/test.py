...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    if cfg.PYTORCH_VERSION_LESS_THAN_040:
        inputs['data'] = [Variable(torch.from_numpy(inputs['data']).cuda(), volatile=True)]
        inputs['data_aug'] = inputs['data']
        inputs['data_aug1'] = inputs['data']
        inputs['rois'] = [Variable(torch.from_numpy(inputs['rois']).cuda(), volatile=True)]
        inputs['labels'] = [Variable(torch.from_numpy(inputs['labels']).cuda(), volatile=True)]
    else:
        inputs['data'] = [torch.from_numpy(inputs['data']).cuda()]
        inputs['data_aug'] = inputs['data']
        inputs['data_aug1'] = inputs['data']
        inputs['rois'] = [torch.from_numpy(inputs['rois']).cuda()]
        inputs['labels'] = [torch.from_numpy(inputs['labels']).cuda()]
    ...
    #rotate_scores = return_dict['refine_score'][3].data.cpu().numpy().squeeze()[:, 3]
    #scores += 0.6 * np.expand_dims(rotate_scores, 1).repeat(81, axis=1)
