...
def main():
    ...
    #a = len(torch.cuda.current_device())
    #b = args.batch_size % a
    #print('number_GPU' % (cfg.NUM_GPUS))
    ...
    nonbias_param_names = []
    nograd_param_names = []
    for key, value in IENet.named_parameters():
    ...
    if args.load_ckpt:
        ...
        net_utils.load_ckpt(IENet, checkpoint['model'])
        if args.resume:
    ...
    if args.load_detectron:  #TODO resume for detectron weights (load sgd momentum values)
        ...
        load_detectron_weight(IENet, args.load_detectron)

    lr = optimizer.param_groups[0]['lr']  # lr of non-bias parameters, for commmand line outputs.

    IENet = mynn.DataParallel(IENet, cpu_keywords=['im_info', 'roidb'],
                                 minibatch=True)
    ...
    try:
        ...
        for step in range(args.start_step, cfg.SOLVER.MAX_ITER):
            ...
            if (step+1) % CHECKPOINT_PERIOD == 0:
                save_ckpt(output_dir, args, step, train_size, IENet, optimizer)
